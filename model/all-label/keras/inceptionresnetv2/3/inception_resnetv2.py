...
IMAGE_STD_FILE = path.get_image_std_file(K_FOLD_FILE, VAL_INDEX, RESOLUTION, rescale=1,
                                         data_type=[config.DATA_TYPE_ORIGINAL, config.DATA_TYPE_SEGMENTED])
...
def get_model(freeze_layers=None, lr=0.001):
    base_model = keras.applications.InceptionResNetV2(weights="imagenet", include_top=False, pooling="max")
    x = base_model.output
...
def check_mean_std_file(datagen: data_loader.KerasGenerator):
    ...
    if not os.path.exists(IMAGE_STD_FILE) or not os.path.exists(IMAGE_STD_FILE):
        datagen.calc_image_mean_std(train_files, 1, RESOLUTION)
...
model = get_model(freeze_layers=249, lr=0.0001)
...
evaluate(model, val_files, y_valid)
model.load_weights("./record/val1/weights.0010.hdf5")
evaluate(model, val_files, y_valid)
model.load_weights("./record/val1/weights.0011.hdf5")
evaluate(model, val_files, y_valid)
