...
+ 重新评估
  ...
  + 删除所有evaluate.txt中，为万辉路径的模型（OK）

+ 对segmented效果很差的图片进行人工修正（放弃此动作，不应该对test集做人工操作而影响预测结果）
...
+ 模型预测
  ...
  + 对非segmented的val1、2进行预测（所有的1， 2我本地都是有的）



+ 人工标注
  + 对测试集进行简单的标注，用于判断模型的输出是否正确（一共只有五次提交机会，没有试错的机会，因此在本地进行一些判断，但是这些标注均不用于训练或者提交）
